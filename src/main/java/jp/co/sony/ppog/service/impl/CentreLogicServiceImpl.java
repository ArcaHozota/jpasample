...
public class CentreLogicServiceImpl implements CentreLogicService {
    ...
    public Page<CityInfoDto> getPageInfo(final Integer pageNum, final String keyword) {
        ...
        if (StringUtils.isNotEmpty(keyword)) {
            ...
            if (!findByNations.isEmpty()) {
                final List<CityInfoDto> getByNations = this.cityViewRepository.getByNations(keyword, pageRequest).getContent().stream().map(item -> {
                    final CityInfoDto cityInfoDto = new CityInfoDto();
                ...
                // ページング検索；
                final List<CityInfoDto> findByNames = this.cityViewRepository.getByNames(keyword, pageRequest).getContent().stream().map(item -> {
                    final CityInfoDto cityInfoDto = new CityInfoDto();
                    BeanUtils.copyProperties(item, cityInfoDto);
                    final String language = this.findLanguageByCty(item.getNation());
                    cityInfoDto.setLanguage(language);
    ...
    public List<String> getListOfNationsById(final Long id) {
        ...
        list.add(nationName);
        final List<String> collect = nations.stream().filter(item -> StringUtils.isNotEqual(item, nationName)).collect(Collectors.toList());
        list.addAll(collect);
        return list;
    }
    ...
    public void update(final CityInfoDto cityInfoDto) {
        ...
        this.cityRepository.updateById(city.getId(), city.getName(), city.getCountryCode(), city.getDistrict(), city.getPopulation());
    }
    ...
    public String findLanguageByCty(final String nationVal) {
        ...
        if (languages.size() == 1) {
            ...
            final List<Language> officialLanguages = languages.stream().filter(al -> StringUtils.isEqual("True", al.getIsOfficial())).collect(Collectors.toList());
            final List<Language> typicalLanguages = languages.stream().filter(al -> StringUtils.isEqual("False", al.getIsOfficial())).collect(Collectors.toList());
    ...
    public List<City> checkDuplicate(final String cityName) {
        ...
        final ExampleMatcher matcher = ExampleMatcher.matching().withStringMatcher(ExampleMatcher.StringMatcher.EXACT).withIgnoreCase(true).withMatcher(cityName, GenericPropertyMatchers.exact()).withIgnorePaths("id", "countryCode", "district", "population", "isDeleted");
